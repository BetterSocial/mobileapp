...
const CreatePost = () => {
    ...
    const [formattedContent, setFormatText] = React.useState('');
    const [formattedMention, setFormatMention] = React.useState('');
    ...
    const mentionUserHandle = (text) => {
        if (text.includes('@')) {
            let position = text.lastIndexOf('#', positionEndCursor);
            let spaceStatus = text.includes(' ', position);
            let detectEnter = text.includes('\n', position);
            let textSeacrh = text.substring(position + 1);
        ...
        handleMention(text, setFormatText);
    }
                                ...
                                {topicSearch.map((item, index) => {
                                    return (
                                        <TouchableNativeFeedback onPress={() => {
                                            ...
                                            setPositionKeyboard('never')
                                            handleHastag(newMessage, setFormatText)

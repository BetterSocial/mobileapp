...
import {Animated, InteractionManager, StatusBar, StyleSheet, Text} from 'react-native';
...
import {useFeedInfiniteQuery, useUpvoteMutation} from './hooks/useNewFeed';
...
const FeedScreen = (props) => {
  ...
  const {data, isLoading: infiniteLoading} = useFeedInfiniteQuery();
  ...
  const setUpVoteHandle = async (post, index) => {
    await upvoteMutation.mutateAsync({post, index});
  };

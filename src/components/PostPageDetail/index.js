...
const PostPageDetailIdComponent = (props) => {
  ...
  const route = useRoute()
  const { item, initComment, initial, getDetailFeed, updateParentPost, updateFeed, onComment, onCommentButtonClicked, findCommentAndUpdate, onNewPollFetched, onPressDownVoteHandle, onPressUpvoteHandle, handleRefreshComment, handleRefreshChildComment, checkVotes, updateVoteLatestChildren, loading, isReaction, setTextComment, textComment, profile, totalComment, totalVote, voteStatus, loadingPost, commentList, scrollViewRef, refBlockComponent, navigateToReplyView} = usePostDetail({...props, route})
  React.useEffect(() => {
    initComment()

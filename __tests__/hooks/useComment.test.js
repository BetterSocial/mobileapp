...
describe('useComment should run correctly', () => {
    ...
    it('upvote status upvote should run correctly', async () => {
        ...
        const findCommentAndUpdate = jest.fn()
        const spy = jest.spyOn(serviceVote, 'voteComment')
        const {result} = renderHook(() => useComment({comment: {...comment, data: {...comment.data, count_upvote: 1, count_downvote: 0}}, level, updateVote, findCommentAndUpdate}))
        ...
        const findCommentAndUpdate = jest.fn()
        const spy = jest.spyOn(serviceVote, 'voteComment')
        const {result} = renderHook(() => useComment({comment: {...comment, data: {...comment.data, count_upvote: 0, count_downvote: 2}}, level, updateVote, findCommentAndUpdate}))

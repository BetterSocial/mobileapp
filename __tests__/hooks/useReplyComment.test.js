...
const updateFeedData = {
    ...
    object: "{\"feed_group\":\"main_feed\",\"message\":\"new #kispray again\",\"profile_pic_path\":\"https://res.cloudinary.com/hpjivutj2/image/upload/v1660620884/nrfnzuhcrozz9v34ngv3.jpg\",\"real_name\":null,\"topics\":[\"kispray\"],\"username\":\"Fajarism\",\"verb\":\"tweet\"}",
    origin: null
}
...
describe('hooks function should run correctly', () => {
    ...
    it('isLastInParent should run correctly', () => {
       ...
       it('updatevote reply should update children vote', async () => {
        ...
        expect(result.current.findCommentAndUpdateHook('51d6e8b4-6ba2-4d5b-a843-a3adb58f9739', data)).toStrictEqual([{
            activity_id: "029ec53e-281d-11ed-b3e4-0a6648bb8f8d",
            data: {
                count_downvote: 0,
                count_upvote: 0,
    ...
    it('updateVoteLatestChildrenParentHook should run correctly', () => {
        ...
        it('updateReplyPostHook should run correctly', async () => {
            ...
            expect(result.current.item).toEqual({"latest_children": {"comment": [{"children_counts": {"comment": 1}, "id": "123", "latest_children": {"comment": [{"text": "superman"}]}}]}})
            await result.current.updateReplyPost(newComment, itemParentProps, commenId2)
            expect(result.current.item).toEqual({"latest_children": {"comment": [{"children_counts": {"comment": 1}, "id": "123", "latest_children": {"comment": [{"text": "agita"}]}}]}})

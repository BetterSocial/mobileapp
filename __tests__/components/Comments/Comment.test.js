...
describe('Comment test should run correctly', () => {
    ...
    const photo = 'https://res.cloudinary.com/hpjivutj2/image/upload/v1659099243/pbdv3jlyd4mhmtis6kqx.jpg'
    const level = 1
    ...
    it('Should match snapshot', () => {
        ...
        const {toJSON} = render(<Comment user={user} comment={comment} photo={photo} level={level} onPress={onPress} findCommentAndUpdate={findComment} updateVote={updateVote} time={time} />)
    ...
    it('btnDownvote should run correctly', () => {
        const onPress = jest.fn()
        const findComment = jest.fn()
        const updateVote = jest.fn()
        const time = '2023-01-09T14:13:31.890300Z'
        ...
        const {getByTestId} = render(<Comment user={user} comment={comment} photo={photo} level={level} onPress={onPress} findCommentAndUpdate={findComment} updateVote={updateVote} time={time} />)
        fireEvent.press(getByTestId('btnDownvote'))
...
describe('useComment should run correctly', () => {
    ...
    it('iVote function should run correctly', async () => {
        const spy = jest.spyOn(serviceVote,'iVoteComment').mockResolvedValue({code: 200, data: {action: 'upvote'}})
        const updateVote = jest.fn()

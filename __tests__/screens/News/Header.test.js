...
import Header from '../../../src/screens/NewsScreen/Header'
...
describe('Header news should run correctly', () => {
   const image ='https://assets.guim.co.uk/images/eada8aa27c12fe2d5afa3a89d3fbae0d/fallback-logo.png'
     const item = {
        actor: "[]",
        content: {
            author: "",
    ...
    it('should match snapshot', () => {
        const {toJSON} = render(<Header image={image} item={item}  domain={url} time={time} />, {wrapper: Store})
        expect(toJSON).toMatchSnapshot()
    })

    it('onPressHeader should run correctly', () => {
        const {getByTestId} = render(<Header image={image} item={item}  domain={url} time={time} />, {wrapper: Store})
        fireEvent.press(getByTestId('headerBtn'))

...
describe('Render block list should run correctly', () => {
    ...
    it('should match snapshot', () => {
        ...
        const {toJSON} = render(<RenderBlockList item={item} onPressBody={onPressBody} onPressList={onPressList} />)
    ...
    it('pressbody should open pressbody function', () => {
         const onPressList = jest.fn()
        const onPressBody = jest.fn()
        const {getByTestId, getAllByTestId} = render(<RenderBlockList item={item} onPressBody={onPressBody} onPressList={onPressList} />)
        act(() => {
            fireEvent.press(getByTestId('pressbody'))
        })
        expect(onPressBody).toHaveBeenCalled()
        expect(getAllByTestId('images')).toHaveLength(1)
        const {getAllByTestId: noItemImageId} = render(<RenderBlockList item={itemNoImage} onPressBody={onPressBody} onPressList={onPressList} />)
    ...
    it('name should show correctly', () => {
          const onPressList = jest.fn()
        const onPressBody = jest.fn()
        const {getByTestId} = render(<RenderBlockList item={item} onPressBody={onPressBody} onPressList={onPressList} />)
        expect(getByTestId('name').props.children[1]).toEqual(item.name)
        const {getByTestId:hashtagId} = render(<RenderBlockList item={item} isHashtag={true} onPressBody={onPressBody} onPressList={onPressList} />)
    ...
    it('Button Block should correctly', () => {
        ...
        const {getAllByTestId, getByTestId} = render(<RenderBlockList handleSetBlock={handleSetBlock} handleSetUnblock={handleSetUnblock} item={item} onPressBody={onPressBody} onPressList={onPressList} />)
        ...
        const {getAllByTestId:getBlockId, getByTestId: getIdSingleBlock} = render(<RenderBlockList handleSetBlock={handleSetBlock} handleSetUnblock={handleSetUnblock}  item={itemNoImage} onPressBody={onPressBody} onPressList={onPressList} />)

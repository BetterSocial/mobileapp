...
describe('Testing Feed Screen Content Link', () => {
    ...
    it('CardHeader Matches Snapshot', () => {
        const tree = renderer.create(<Header domain={og.domain} image={og.domainImage} date={og.date} score={null}/>).toJSON()
        expect(tree).toMatchSnapshot()
    })
    ...
    it('Render default domain image if domainImage is null', () => {
        const {getByTestId} = render(<Header domain={og.domain} image={og.domainImage} date={og.date} score={null}/>)
    ...
    it('Render domain image if domainImage is set', () => {
        const {getByTestId} = render(<Header domain={og.domain} image={'https://res.cloudinary.com/hpjivutj2/image/upload/v1617245336/Frame_66_1_xgvszh.png'} date={og.date} score={null}/>)
    ...
    it('Will navigate to domain page when header is clicked', () => {
        ...
        const {getByTestId} = render(<Card onHeaderPress={onHeaderPress} domain={og.domain} date={og.date} description={og.description} domainImage={og.domainImage} image={og.image} title={og.title} url={og.url}/>)
    ...
    it('Will navigate to context link page when content is clicked', () => {
        ...
        const {getByTestId} = render(<Card onCardContentPress={onContentPress} domain={og.domain} date={og.date} description={og.description} domainImage={og.domainImage} image={og.image} title={og.title} url={og.url}/>)

...
describe('Content poll should same as snapshot', () => {
    ...
    it('renderPercentageBar should run correctly', () => {
        const {getAllByTestId} = render(<PollOptinComponent mypoll={[]} item={item.pollOptions[0]} selectedindex={'1'}  multiplechoice={false} isexpired={true}   isalreadypolling={true}  />);
        expect(getAllByTestId(`isExpired`)).toHaveLength(1)
        const {getAllByTestId:getAlreadyPolling} = render(<PollOptinComponent mypoll={[]} item={item.pollOptions[0]} selectedindex={'1'}  multiplechoice={false} isexpired={false}   isalreadypolling={true}  />);
        expect(getAlreadyPolling('isAlreadyPolling')).toHaveLength(1)
        
    })

    it('renderPollBadge should run correctly', () => {
        const {getAllByTestId} = render(<PollOptinComponent maxpolls={['c4d16d91-89f5-423c-acb5-95a1b91ec53b']} mypoll={[]} item={item.pollOptions[0]} selectedindex={'1'}  multiplechoice={false} isexpired={true}   isalreadypolling={true}  />);
        expect(getAllByTestId('pollWinner')).toHaveLength(1)
        const {getAllByTestId:getPollNotEndedAndIsMine} = render(<PollOptinComponent maxpolls={['c4d16d91-89f5-423c-acb5-95a1b91ec53b']} mypoll={[{polling_option_id: 'c4d16d91-89f5-423c-acb5-95a1b91ec53b'}]} item={item.pollOptions[0]} selectedindex={'1'}  multiplechoice={false} isexpired={false}   isalreadypolling={true}  />);
        expect(getPollNotEndedAndIsMine('isPollNotEndedAndIsMine')).toHaveLength(1)
        const {getAllByTestId:getNoPoll} = render(<PollOptinComponent maxpolls={['c4d16d91-89f5-423c-acb5-95a1b91ec53a']} mypoll={[{polling_option_id: 'c4d16d91-89f5-423c-acb5-95a1b91ec53a'}]} item={item.pollOptions[0]} selectedindex={'1'}  multiplechoice={false} isexpired={false}   isalreadypolling={true}  />);
    ...
    it('option text should be there', () => {
        const  onnewpollfetched = jest.fn()
        const {getAllByText} = render(<ContentPoll item={item} onnewpollfetched={onnewpollfetched} multiplechoice={item.multiplechoice} pollexpiredat={moment().add(2, 'day')} voteCount={item.voteCount} isalreadypolling={false} polls={item.pollOptions} />);
    ...
    it('optionPercentage should count correctly', () => {
         const  onnewpollfetched = jest.fn()
        const {getAllByText} = render(<ContentPoll item={item} onnewpollfetched={onnewpollfetched} multiplechoice={item.multiplechoice} pollexpiredat={moment().subtract(2, 'day')} voteCount={item.voteCount} isalreadypolling={true} polls={item.pollOptions} />);
    ...
    it('isExpiredPollOption should have length', () => {
            const  onnewpollfetched = jest.fn()
        const {getAllByTestId} = render(<ContentPoll item={itemNotMultiple} onnewpollfetched={onnewpollfetched} multiplechoice={itemNotMultiple.multiplechoice} pollexpiredat={moment().subtract(2, 'day')} voteCount={itemNotMultiple.voteCount} isalreadypolling={true} polls={itemNotMultiple.pollOptions} />);
